    ...
    if (0 == mix_prompt_trigger_ticks)
    {
        if ((playback_interval_in_ticks > 0)&&(playback_last_irq_ticks > 0))
        {
            mix_prompt_trigger_ticks = playback_last_irq_ticks+
                (get_prompt_trigger_delay(promptId)/playback_interval_in_ticks)*playback_interval_in_ticks;
            TRACE("playback_last_irq_ticks %d",playback_last_irq_ticks);
            TRACE("playback_interval_in_ticks %d mix_prompt_trigger_ticks %d",playback_interval_in_ticks, mix_prompt_trigger_ticks);
            return true;
        }
    }
...
//    TRACE("irq curr ticks:%d/%d %d/%d",curr_ticks, bt_syn_get_curr_ticks(conhandle),
//                                 btclk, btdrv_syn_get_curr_ticks());
    ...
    if (mix_prompt_trigger_ticks > 0)
    {
        ...
        {
            TRACE("ticks<1> %d - trigger ticks %d", curr_ticks,mix_prompt_trigger_ticks);
            tws_enable_mix_prompt(true);
        }
        else if ((curr_ticks < mix_prompt_trigger_ticks) && 
                ((mix_prompt_trigger_ticks - curr_ticks) < PROMPT_TICKS_OFFSET_TO_TRIGGER_MIX))
        {
            TRACE("ticks<2> %d - trigger ticks %d", curr_ticks,mix_prompt_trigger_ticks);
            tws_enable_mix_prompt(true);
        }
    }
...
void app_tws_cmd_sync_mix_prompt_req_handler(uint8_t* ptrParam, uint16_t paramLen)
{
    ...
    //TRACE("pReq->promptId,pReq->trigger_time:0x%x,%d", pReq->promptId, pReq->trigger_time);
...
void audio_prompt_clear_a2dp_pending_stop_stream_op(void)
{
    if (PENDING_TO_STOP_A2DP_STREAMING == audio_prompt_env.pendingStopOp)
    {
        TRACE("will clear the PENDING_TO_STOP_A2DP_STREAMING");
        audio_prompt_env.pendingStopOp = PENDING_TO_STOP_INVALID;
        audio_prompt_env.savedStoppedStreamId = -1;
    }
}

void audio_prompt_clear_sco_pending_stop_stream_op(void)
{
    if (PENDING_TO_STOP_SCO_STREAMING == audio_prompt_env.pendingStopOp)
    {
        TRACE("will clear the PENDING_TO_STOP_SCO_STREAMING");
        audio_prompt_env.pendingStopOp = PENDING_TO_STOP_INVALID;
        audio_prompt_env.savedStoppedStreamId = -1;
    }
}
    ...
    uint8_t* promptDataPtr, uint32_t promptDataLen)
{
    ...
    TRACE("start audio prompt. target sample rate %d", targetSampleRate);
...
bool audio_prompt_start_playing(uint16_t promptPram, uint32_t targetSampleRate)
{
   ...
   // promptId = PROMPT_ID_FROM_ID_VALUE(promptId); 
    ...
    TRACE("start audio prompt. target sample rate %d id:%d pram:%04x", targetSampleRate, promptId, PROMPT_PRAM_FROM_ID_VALUE(promptPram));
...
void audio_prompt_stop_playing(void)
{
    ...
    {        
        if (PENDING_TO_STOP_A2DP_STREAMING == audio_prompt_env.pendingStopOp)
        {
            TRACE("Stop the pending stopped a2dp media stream.");
        ...
        {
            TRACE("Stop the pending stopped sco media stream.");
