...
#define Sha256_CheckParm(code, exp) if (!(exp)) return (code)
...
uint32_t _sha256_update(sha256_context_t *ctx, const uint8_t * data, size_t len)
{
    Sha256_CheckParm(SHA256_ERROR_NULL, ctx != 0);
    if (((len > 0) && (data == NULL)))
    {
        return SHA256_SUCCESS;
    }
...
uint32_t _sha256_final(sha256_context_t *ctx, uint8_t * hash, uint8_t le)
{
    uint32_t i;
    Sha256_CheckParm(SHA256_ERROR_NULL, ctx != 0);
    Sha256_CheckParm(SHA256_ERROR_NULL, hash != 0);
