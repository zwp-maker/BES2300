...
#define SET_SLAVE_BLE_STATE(newState)                                                                                     \
    do                                                                                                              \
    {                                                                                                               \
        TRACE("[BLE][STATE]%s->%s at line %d", g_slave_ble_state_str[slaveBleMode.state], g_slave_ble_state_str[newState], __LINE__); \
...
#define SET_SLAVE_BLE_OP(newOp)                                                                            \
    do                                                                                               \
    {                                                                                                \
        TRACE("[BLE][OP]%s->%s at line %d", g_slave_ble_op_str[slaveBleMode.op], g_slave_ble_op_str[newOp], __LINE__); \
...
void app_ibrt_slave_ble_cmd_complete_callback(const void *para)
{
    ...
    switch (p_cmd_complete->cmd_opcode)
    {
        case HCI_BLE_ADV_CMD_OPCODE:
            ...
            {
                TRACE("%s error p_cmd_complete->param[0] %d", __func__, p_cmd_complete->param[0]);
...
{
    TRACE("ble adv start with adv_type %d advIntervalms %dms", adv_type, advInterval);
