...
static void pc_usb_cmd_set (struct USB_AUDIO_VENDOR_MSG_T* msg)
{
    ...
    s_cur_cmd = cmd_id;
    //TRACE("%s: cmd[%s], id[%d]", __func__, s_pc_cmd_lst[cmd_id], s_cur_cmd);
	...
	case PC_TOOL_CMD_ANC_MC_EQ_TUNING:
        //TRACE("recev len:%d",msg->length);
        /*
      	       TRACE("***********recev test*************");
               for(int i=0;i<msg->length;i++)
               {
                     TRACE("msg->data[%d]:0x%x",i,msg->data[i]);
               }
               TRACE("***********recev test*************");
               */
...
{
    //TRACE("%s: cmd[%d]", __func__, s_cur_cmd);
