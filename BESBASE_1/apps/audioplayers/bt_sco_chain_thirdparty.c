...
int speech_tx_process(void *pcm_buf, void *ref_buf, int *pcm_len)
{
    ...
    for (int i = 0, j = 0; i < pcm16_len; i += SPEECH_CODEC_CAPTURE_CHANNEL_NUM, j++)
        tmp_buf[j] = pcm16_buf[i];
    audio_dump_add_channel_data(1, tmp_buf, pcm16_len/SPEECH_CODEC_CAPTURE_CHANNEL_NUM);

    for (int i = 1, j = 0; i < pcm16_len; i += SPEECH_CODEC_CAPTURE_CHANNEL_NUM, j++)
        tmp_buf[j] = pcm16_buf[i];
    audio_dump_add_channel_data(2, tmp_buf, pcm16_len/SPEECH_CODEC_CAPTURE_CHANNEL_NUM);
